...
//    private List<CartDataList> cartDataLists;
    ...
    protected void initView() {
        ...
        rlt_cart_data = (RelativeLayout) findViewById(R.id.rlt_cart_data);
        sv_cart_none = (ScrollView)findViewById(R.id.sv_cart_none);
        cart_hot_list = (MyGridView)findViewById(R.id.cart_hot_list);
        mlv_cart_store = (MyListView)findViewById(R.id.mlv_cart_store);
        btn_all_select = (CheckBox)findViewById(R.id.btn_all_select);
        btn_commit_order = (Button)findViewById(R.id.btn_commit_order);
        tv_all_price = (TextView)findViewById(R.id.tv_all_price);
        tv_clear_no_use_goods = (TextView)findViewById(R.id.tv_clear_no_use_goods);
        Constant.allselect=1;
        ...
        cart_hot_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                startActivity(new Intent(ShoppingCartActivity.this, GoodsDetailActivity.class)
    ...
    private void initData() {
        progressHUD = ProgressHUD.show(ShoppingCartActivity.this, "读取中", true, null);
        final PotatoService service = RTHttpClient.create(PotatoService.class);
        service.selectUserCart(SharePreferencesUtils.getBtdToken(ShoppingCartActivity.this),"CNY",
                new CallbackSupport<CartData>(progressHUD,ShoppingCartActivity.this) {
            @Override
            public void success(CartData cartData, Response response) {
                ...
                if (cartData.getData().isCartEmpty()){
                    ...
                    if (!cartData.getData().getGoodsList().isEmpty()){
                        ...
                        adapter = new HeadListAdapter(headLists,ShoppingCartActivity.this, ImageLoader.getInstance());
...
//                    tv_all_price.setText(all_price);
                    indentAdapter = new IndentAdapter(ShoppingCartActivity.this,handler,layoutInflater);
...
//        Log.i("order_form", JSONObject.toJSONString(Constant.FORM));
        if (Constant.map.size()==0){
            ToastUtils.showToastCenter(ShoppingCartActivity.this,"请选择下单商品~");
            return;
        }
        final ProgressHUD progressHUD = ProgressHUD.show(ShoppingCartActivity.this, "提交中", true, null);
        final PotatoService service = RTHttpClient.create(PotatoService.class);
        service.confirmOrderInfo(SharePreferencesUtils.getBtdToken(ShoppingCartActivity.this),getStringMap(),
                "CNY", new CallbackSupport<OrderDetail>(progressHUD,ShoppingCartActivity.this) {
            @Override
            public void success(OrderDetail orderDetail, Response response) {
                ...
                if (orderDetail.getStatus().equals("1")){
                    ...
                    if (orderDetail.getErrorCode().equals("51103")){
                        DialogUtils.showDialogs(ShoppingCartActivity.this.getLayoutInflater(),
                                ShoppingCartActivity.this,orderDetail.getData().getErrorDepotName());
                    }else if (orderDetail.getErrorCode().equals("51105")){
                        if (orderDetail.getData().getErrorGoodsList().get(0).getGoodsError().equals("51014")){
                            ToastUtils.showToastCenter(ShoppingCartActivity.this,"库存不足");
                        }else {
                            ToastUtils.showToastCenter(ShoppingCartActivity.this,"存在失效商品");
                            initData();
                        }
                    }else {
...
//        }
    ...
    public String getStringMap(){
        ...
        for (Map.Entry<String, String> entry : Constant.map.entrySet()) {
            ...
            Log.d("ordermap", entry.getKey()+"-----"+entry.getValue());
        }
    ...
    private void clearNoUseGoods() {
        final PotatoService service = RTHttpClient.create(PotatoService.class);
        service.cleanDisabledCartGoods(SharePreferencesUtils.getBtdToken(this),new Callback<Success>() {
            @Override
